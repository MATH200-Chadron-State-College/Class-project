...
The final exam is more meaningful to consider, since it always covers the same basic material.
```{r}
summary(Final)
```

```{r}
recent <- student_data[ which(Year > 2020),]
summary(recent$Final)
```

### Participation vs. Final Exam Grade

Hopefully, student participation is corrolated with student success. We select the final exam as a measure of student success (rather than the course grade), because participation is included in the course grade; it's fatuous to talk about them being corrolated.

```{r}
student_data$Participation<-unlist(Participation)
student_data$Final<-unlist(Final)
ggplot(student_data, aes(Participation, Final)) +    geom_point()
cor(Participation,Final)
```

Looking at the data, students who give up before they take the final exam will obviously not be impacted by Participation, hence the points on the horizontal axis. Let's try removing those from the picture.
